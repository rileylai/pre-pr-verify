...
The command writes canonical ChangeSet JSON to stdout unless `--output` is explicitly supplied. It performs no code review and emits no readiness verdict. The Python library also provides bounded requirement and Standards discovery plus typed verification planning, capability decisions, disposable-snapshot execution, and deterministic result evidence. Semantic review, ReviewArtifact reduction/reporting, and GitHub integration remain unimplemented.
